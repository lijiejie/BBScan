    ...
    if not args.no_scripts:
        ...
        for _script in glob.glob('scripts/*.py'):
            ...
            if script_name.startswith('_'):
                continue
            with open(_script, 'rb') as f:content = f.read()
        ...
        try:
            ...
            if 'self.http_request' in content or 'self.conn_pool.urlopen' in content:
                args.require_no_http = False  # Plugin depends on an HTTP connection pool
            if 'self.index_' in content:
