        ...
        try:
            with open(file_path, 'r', encoding=encoding, errors='ignore') as file:
                for _line in file:
                    ...
                    if _m:
                        self.text_to_find.append(_m.group())
                    else:
                        _m = re_regex_text.search(_line)
                        if _m:
                            self.regex_to_find.append(re.compile(_m.group(1)))
        except UnicodeDecodeError:
            self.print_msg('[ERROR] Unable to decode file: %s' % file_path)
            return
...
def domain_lookup_check(queue_targets_origin, processed_targets, queue_targets, q_results):
    ...
    while True:
        ...
        if url.find('://') < 0:
            ...
            scheme, netloc, path, params, query, fragment = urllib.parse.urlparse(url, 'http')
    ...
    if args.network != 32:
        ...
        for ip in processed_targets:
            if ip.find('/') > 0:    # 网络本身已经处理过
                continue
            _network = '%s/%s' % ('.'.join(ip.split('.')[:3]), args.network)
            if _network in processed_targets:
                continue
            processed_targets.append(_network)

            if args.network >= 20:
                sub_nets = [ipaddress.IPv4Network('%s/%s' % (ip, args.network), strict=False).hosts()]
            else:
                sub_nets = ipaddress.IPv4Network('%s/%s' % (ip, args.network), strict=False).subnets(new_prefix=22)
